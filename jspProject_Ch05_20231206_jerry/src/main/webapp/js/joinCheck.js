 ...
 */
 ...
 function joinCheck(){
	 ...
	 if(document.joinForm.memberid.value.length < 4 || document.joinForm.memberid.value.length > 10){
		 alert('아이디는 4글자 이상 10글자 이하로만 만드실 수 있습니다. 다시 시도해 주세요.');
		 return false;
		 
	 }
	
	 
	 /* 비밀번호가 공란으로 입력되었는지 유효성 검사*/
	...
	/*비밀번호는 숫자만 입력가능하도록 유효성 검사*/
	 var numPassword = /^[0-9]*$/;
	 if(!numPassword.test(document.joinForm.memperpw.value)){
		 alert('비밀번호는 숫자만 입력가능합니다. 다시 확인해 주세요.');
		 return false;
	 }
	 
	  /* 이름이 공란으로 입력되었는지 유효성 검사*/
